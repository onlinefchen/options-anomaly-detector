...
class HybridDataFetcher:
    ...
    def enrich_with_leap_cp(self, data: List[Dict], top_n: int = 30, trading_date: Optional[str] = None):
        """
        Enrich top N tickers with LEAP C/P ratio from API (volume-based, not OI)

        This method fetches contract data from API to calculate LEAP C/P ratio
        based on volume, without updating OI fields. Useful for historical dates
        where OI data would be stale but volume-based LEAP ratio is still valuable.

        Args:
            data: List of ticker data dicts (must be pre-sorted by volume)
            top_n: Number of top tickers to enrich with LEAP C/P
            trading_date: Trading date in YYYY-MM-DD format for LEAP C/P calculation

        Returns:
            Tuple of (enriched_data, enriched_count)
        """
        from options_utils import calculate_leap_cp_ratio
        ...
        for idx, item in enumerate(data[:top_n], 1):
            ...
            if api_response.get('status') == 'OK':
                ...
                # Calculate LEAP C/P ratio based on volume
                leap_cp = calculate_leap_cp_ratio(contracts, trading_date)
                item['leap_cp_ratio'] = leap_cp
